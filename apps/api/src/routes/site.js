...
// The surprise pages render their interactivity as an inline <script>. Helmet's
// default CSP blocks inline scripts/event-handlers, which would leave the
// navigation buttons dead. We issue a per-request nonce so the inline script is
// allowed, and permit inline event handlers (the playful template uses
// onclick="goTo(n)"). Everything else stays locked down.
function surpriseCSP(nonce) {
  return [
    "default-src 'self'",
    "base-uri 'self'",
...
router.get("/:slug", async (req, res) => {
  try {
    ...
    res.set("Content-Security-Policy", surpriseCSP(nonce));
